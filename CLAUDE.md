...
- `i18n.ts` — inline `DICTS` object, one dict per language, with `t(key, params?)` helper; `localStorage`-persisted; subscribe via `onLangChange(cb)`. `en` and `hu` are hand-maintained; every other language is filled in automatically by `KO_language_translator/main.py` (Google Translate). The **Translate i18n** workflow (`.github/workflows/translate-i18n.yml`) runs it on every push that touches `i18n.ts` and commits the result back with `[skip ci]` (so it never starts a release). The release workflow (`release.yml`) does **not** translate itself — its `release` job has a "Wait for i18n auto-translation, then sync to latest main" step (`id: sync`) that polls the GitHub API for the Translate i18n run of the triggering commit, waits for it to finish (2-min appear window, 20-min hard cap), then `git reset --hard origin/main` and exports that SHA as `build_sha`, which every downstream build job checks out — so every published bundle ships the complete dictionary. Runner for the standalone workflow is declared via the `RUNNER_TRANSLATE` repo variable (any self-hosted OS), GitHub-hosted fallback.
