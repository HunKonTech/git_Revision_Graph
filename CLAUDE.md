...
- `i18n.ts` — inline `DICTS` object, one dict per language, with `t(key, params?)` helper; `localStorage`-persisted; subscribe via `onLangChange(cb)`. `en` and `hu` are hand-maintained; every other language is filled in automatically by `KO_language_translator/main.py` (Google Translate). The **Translate i18n** workflow (`.github/workflows/translate-i18n.yml`) runs on **every** push (no `paths:` filter) and is the single entry point of the release chain: **push → Translate i18n → Build & Publish Extensions (`release.yml`) → Deploy demo (`deploy-demo.yml`)**, each link wired by `workflow_run` on the previous one's completion (neither `release.yml` nor `deploy-demo.yml` has a direct `push` trigger any more). When a push doesn't touch `i18n.ts` / `KO_language_translator/` / the workflow itself, Translate i18n's "Decide whether translation is needed" step (`id: need`) short-circuits the heavy steps but the workflow still finishes green so the chain fires. When it does translate, it commits the result back with `[skip ci]` (which would otherwise re-trigger the chain). `release.yml` only proceeds when the upstream run's `conclusion == 'success'` (or on manual `workflow_dispatch`); its `release` job's "Sync to latest main" step (`id: sync`) just does `git reset --hard origin/main` (the translation commit, if any, is already there — no polling) and exports that SHA as `build_sha`, which every downstream build job checks out. Runner for the Translate i18n workflow is declared via the `RUNNER_TRANSLATE` repo variable (any self-hosted OS), GitHub-hosted fallback.
