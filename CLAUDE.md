...
- `i18n.ts` — inline `DICTS` object, one dict per language, with `t(key, params?)` helper; `localStorage`-persisted; subscribe via `onLangChange(cb)`. `en` and `hu` are hand-maintained; every other language is filled in automatically by `KO_language_translator/main.py` (Google Translate). The **Translate i18n** workflow (`.github/workflows/translate-i18n.yml`) runs it on every push that touches `i18n.ts` and commits the result back with `[skip ci]`; the release workflow (`release.yml`, `translate` step in the `release` job) re-runs it before building so every published bundle ships the complete dictionary — all downstream build jobs check out that job's `build_sha`. Runner for the standalone workflow is declared via the `RUNNER_TRANSLATE` repo variable (any self-hosted OS), GitHub-hosted fallback.
