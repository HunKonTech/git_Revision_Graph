...
const DICTS: Record<string, Partial<Dict>> & { en: Dict } = {
  en: {
    ...
    "settings.svnBranchDialogHint": "Show a folder-tree picker when creating a branch (instead of a simple prompt).",
    ...
    "settings.gitSourceCustomHint": "Specify the path to your own git executable (e.g. /usr/bin/git or C:\\â¦\\git.exe).",
    ...
    "settings.mergeModeMergeHint": "Merges the branch as it is: its commits become part of the current branch's history, joined by a merge commit (or fast-forwarded when possible).",
    "settings.mergeModeSquashHint": "Collapses the whole branch into a single commit on the current branch (git merge --squash). The branch's own commits never enter the current branch's history — you only see one commit there. The branch itself is left untouched, so its commits stay visible in its own lane.",
    ...
    "settings.mergedViewBranchHint": "A merged branch's commits are drawn only in their own lane; the merge shows up as a connector. The classic view.",
    ...
    "node.mergedInTooltip": "Merge {merge} brought this commit into \"{target}\" â it was written on \"{origin}\".",
    ...
    "merge.squashNote": "Squash merge — every change lands as ONE commit on “{target}”; “{source}”'s own commits stay out of its history. Switch back in Settings › Merge.",
  ...
  hu: {
    ...
    "settings.svnBranchDialogHint": "Branch létrehozásakor mappafa-választó jelenjen meg (egyszerű beírás helyett).",
    ...
    "settings.mergeModeSquash": "Squash merge",
    "settings.mergeModeMergeHint": "Az ágat úgy olvasztja be, ahogy van: a commitjai bekerülnek az aktuális ág történetébe, egy merge commit köti őket össze (vagy fast-forward, ha lehet).",
    "settings.mergeModeSquashHint": "Az egész ágat egyetlen commitba vonja össze az aktuális ágon (git merge --squash). Az ág saját commitjai nem kerülnek be az aktuális ág történetébe — ott csak egy commit látszik. Magát az ágat nem bántja, így annak commitjai a saját oszlopukban továbbra is látszanak.",
    ...
    "settings.mergedViewBranchHint": "A beolvasztott ág commitjai csak a saját sávjukban látszanak, a merge pedig összekötő vonalként. A megszokott nézet.",
    ...
    "node.mergedInTooltip": "Ezt a commitot a(z) {merge} merge hozta be a(z) „{target}” ágba — eredetileg a(z) „{origin}” ágon készült.",
    ...
    "merge.squashBadge": "Squash",
    "merge.squashNote": "Squash merge — minden változás EGYETLEN commitként kerül a(z) „{target}” ágra; a(z) „{source}” saját commitjai nem kerülnek be annak történetébe. A Beállítások › Beolvasztás (merge) alatt állítható vissza.",
  ...
  zh: {
    ...
    "settings.svnBranchDialogHint": "创建分支时显示文件夹树选择器（而非简单的输入框）。",
    ...
    "settings.gitSourceCustomHint": "æå®æ¨èªå·±ç git å¯æ§è¡æä»¶è·¯å¾ï¼ä¾å¦ /usr/bin/git æ C:\\â¦\\git.exeï¼ã",
    ...
    "settings.mergeModeMergeHint": "按原样合并分支：它的提交会成为当前分支历史的一部分，由一个合并提交连接（可以快进时则快进）。",
    "settings.mergeModeSquashHint": "把整个分支压缩成当前分支上的一个提交（git merge --squash）。分支自己的提交不会进入当前分支的历史——那里只看到一个提交。分支本身保持不变，它的提交仍显示在自己的列中。",
    "settings.mergedView": "图中的已合并提交",
    "settings.mergedViewBranch": "仅显示在各自的分支上",
    "settings.mergedViewTarget": "也显示在被合入的分支中",
    "settings.mergedViewBranchHint": "被合并分支的提交只画在自己的泳道里，合并本身显示为一条连接线。这是经典视图。",
    "settings.mergedViewTargetHint": "合并带入的提交同时画在接收分支的泳道中，堆叠在合并提交下方，与原始提交同一行——因为该分支确实已经包含它们。副本使用浅色底并标注它们真正被写入的分支，因此不会被误认为是在此处创建的。Squash 合并不受影响：squash 只写入一个普通提交，因此只会出现那一个提交。",
    "legend.title": "图例",
    ...
    "node.mergedInTooltip": "合并 {merge} 将此提交带入 “{target}” — 它是在 “{origin}” 上创建的。",
    ...
    "merge.squashNote": "压缩合并 —— 所有改动作为“一个”提交落在“{target}”上；“{source}”自己的提交不会进入它的历史。可在“设置 › 合并”中改回。",
  ...
  ru: {
    ...
    "settings.svnBranchDialogHint": "Показывать выбор дерева папок при создании ветки (вместо простого ввода).",
    ...
    "settings.gitSourceCustomHint": "Ð£ÐºÐ°Ð¶Ð¸ÑÐµ Ð¿ÑÑÑ Ðº ÑÐ¾Ð±ÑÑÐ²ÐµÐ½Ð½Ð¾Ð¼Ñ Ð¸ÑÐ¿Ð¾Ð»Ð½ÑÐµÐ¼Ð¾Ð¼Ñ ÑÐ°Ð¹Ð»Ñ git (Ð½Ð°Ð¿ÑÐ¸Ð¼ÐµÑ, /usr/bin/git Ð¸Ð»Ð¸ C:\\â¦\\git.exe).",
    ...
    "settings.mergeModeMergeHint": "Сливает ветку как есть: её коммиты входят в историю текущей ветки и связываются коммитом слияния (или выполняется fast-forward, если возможно).",
    "settings.mergeModeSquashHint": "Схлопывает всю ветку в один коммит в текущей ветке (git merge --squash). Собственные коммиты ветки не попадают в историю текущей ветки — там виден только один коммит. Сама ветка не изменяется, поэтому её коммиты остаются видны в своей колонке.",
    ...
    "settings.mergedViewBranchHint": "Коммиты влитой ветки рисуются только в её собственной дорожке, а слияние — соединительной линией. Классический вид.",
    ...
    "node.mergedInTooltip": "Слияние {merge} принесло этот коммит в «{target}» — он был написан в «{origin}».",
    ...
    "merge.squashBadge": "Squash",
    "merge.squashNote": "Squash-слияние — все изменения попадут в ветку «{target}» ОДНИМ коммитом; собственные коммиты «{source}» не войдут в её историю. Вернуть обычное слияние можно в «Настройки › Слияние».",
